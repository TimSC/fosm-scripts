...
getUidFromToken(token) ; Public get uid from confirmation token
	...
	i '$d(^pendingUserx("emailToken",token)) d  q
	. s exists=0
	. s uid=0
	. q
	;
...
metric	;
	...
	f  d  i c="" q
	. s c=$o(^c(c)) i c="" q
	. s uid=$g(^c(c,"t","@uid")) i uid="" q
	. s name=^user(uid,"name")
	. s timestamp=$g(^c(c,"t","@timestamp")) i timestamp="" q
	. s ccyymm=$p(timestamp,"-",1,2)
	. s ccyy=$p(timestamp,"-",1)
	. s ^userMetric("monthByUser",name,ccyymm)=$g(^userMetric("monthByUser",name,ccyymm))+1
	. s ^userMetric("yearByUser",name,ccyy)=$g(^userMetric("YearByUser",name,ccyy))+1
	. s ^userMetric("month",ccyymm)=$g(^userMetric("month",ccyymm))+1
	. s ^userMetric("year",ccyy)=$g(^userMetric("year",ccyy))+1
	. s ^userMetric("userByMonth",ccyymm,name)=$g(^userMetric("userByMonth",ccyymm,name))+1
	. s ^userMetric("userByYear",ccyy,name)=$g(^userMetric("userByYear",ccyy,name))+1
	q
	

getMetrics()	; Public ; Get user metrics
	;
	n year
	;
	d header^http("text/xml")
	...
	w ">",$c(13,10)
	;
	w "  ","<userByYear>",$c(13,10)
	s year=""
	f  d  i year="" q
	. s year=$o(^userMetric("userByYear",year)) i year="" q
	. w "    ","<year"
	. w $$attribute^osmXml("year",year,"")
	. w $$attribute^osmXml("changesets",$g(^userMetric("year",year)),"")
	. w ">",$c(13,10)
	. ;
	. s user=""
	. f  d  i user="" q
	. . s user=$o(^userMetric("userByYear",year,user)) i user="" q
	. . w "      ","<user"
	. . w $$attribute^osmXml("user",user,"")
	. . w $$attribute^osmXml("changesets",^userMetric("userByYear",year,user),"")	
	. . w "/>",$c(13,10)
	. w "    </year>",$c(13,10)
	w "  ","</userByYear>",$c(13,10)
	;
